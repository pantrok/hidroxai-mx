...
CLI = """Comisión Nacional del Agua
Subdirección General Técnica
Estación: Abasolo
Clave: ABASOLO
Estado: Tamaulipas
...
Fecha, Precipitación(mm), Temperatura Media(ºC), Temperatura Máxima(ºC), Temperatura Mínima(ºC), Evaporación(mm)
2010/01/01,0.00,19.7,26.3,15.7,
2010/01/02,3.40,-,-,-,
"""

CAT_HID = ('Clave,Nombre de la estaci\xf3n,Latitud,Longitud,Altitud,Estado,Municipio,R.H.,Cuenca\n'
           'ABSTP,"Abasolo",24.05,-98.37,100,Tamaulipas,Abasolo,26,Panuco\n')
CAT_CLI = ('N\xfamero,Clave ,Nombre,Latitud,Longitud,Altitud,Estado,Municipio,Id,Cuenca de disponibilidad,'
...
def test_catalog_region(tmp_path):
    ph = tmp_path / "h.csv"; ph.write_text(CAT_HID, encoding="latin-1")
    dh = conagua.read_catalog(ph)
    assert dh["region_hidrologica"].iloc[0] == "26"
    assert abs(dh["latitud"].iloc[0] - 24.05) < 1e-9
    pc = tmp_path / "c.csv"; pc.write_text(CAT_CLI, encoding="latin-1")
