...
def fig1_inventario_cobertura(hid: pd.DataFrame, metrics: dict) -> None:
    ...
    ax[0].set_title("Estaciones hidrométricas por región hidrológica")
    ax[0].set_xlabel("Región hidrológica")
    ax[0].set_ylabel("# estaciones")
    ...
    ax[1].axvline(60, color="orange", ls="--", label="umbral hidro 60%")
    ax[1].axvline(80, color="red", ls="--", label="umbral clima 80%")
    ax[1].set_title("Distribución de cobertura (2010–2025)")
    ax[1].set_xlabel("% días con dato")
    ax[1].set_ylabel("# estaciones")
    ax[1].legend()
    plt.tight_layout()
...
def fig2_descriptiva(hid: pd.DataFrame, metrics: dict) -> None:
    ...
    ax.set_yscale("log")
    ax.set_title("Distribución del gasto medio diario por región hidrológica (escala log)")
    ax.set_ylabel("Gasto medio (m³/s, log)")
    ax.set_xlabel("Región hidrológica")
...
def fig3_calidad(hid: pd.DataFrame, metrics: dict) -> None:
    ...
    ax.set_title("Distribución de banderas de calidad")
    ax.set_ylim(0, 100)
    plt.tight_layout()
...
def fig5_mapa(hid: pd.DataFrame, metrics: dict) -> None:
    ...
    ax.set_title("Ubicación de estaciones hidrométricas seleccionadas")
...
def fig6_temporal(hid: pd.DataFrame, metrics: dict) -> None:
    ...
    ax[0].set_title("Climatología mensual del gasto medio")
    ax[0].set_xlabel("Mes")
    ax[0].set_ylabel("m³/s (media ± 1σ)")
    ...
    ax[1].set_title("Media anual del gasto (líneas: sequías 2011/2021/2023)")
    ax[1].set_xlabel("Año")
    ax[1].set_ylabel("m³/s")
    ax[1].grid(alpha=0.3)
    plt.tight_layout()
...
def fig7_precip_gasto(hid: pd.DataFrame, cli: pd.DataFrame, metrics: dict) -> None:
    ...
    ax.set_title("Correlación rezagada: precip(t-lag) vs gasto(t) — series agregadas nacionales")
    ax.grid(alpha=0.3)
    plt.tight_layout()
...
def fig8_basin_map(metrics: dict) -> None:
    ...
    ax.scatter(sel["longitud"], sel["latitud"], s=8, c="black", alpha=0.5,
               label=f"Estaciones hidro ≥60% (n={len(sel)})", zorder=3)

    ax.set_xlabel("Longitud")
    ax.set_ylabel("Latitud")
    ax.set_title("Cuencas piloto HidroXAI-MX: 123 subcuencas delineadas y bboxes curados")
